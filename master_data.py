...
class MasterStore:
    ...
    def _validate_outlets(df):
        ...
        if df["Outlet Name"].eq("").any(): errors.append("Outlet Name contains blank values.")
        if (~df["Outlet Name"].str.fullmatch(r"[A-Za-z0-9 ]+").fillna(False)).any(): errors.append("Outlet Name must contain alphabets, numbers and spaces only.")
