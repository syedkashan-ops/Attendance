...
def get_worksheet_cached():
    ...
    if not header:
        ...
        if not any(str(x).strip() for x in header):
            _sheet_write(lambda: ws.update("A1:S1", [HEADERS], value_input_option="USER_ENTERED"))
    return ws
...
def complete_out_visit(visit_id, location, tolerance_meters=100, warning_meters=200):
    ...
    ws = get_worksheet_cached()
    _sheet_write(lambda: ws.update(f"K{row_num}:S{row_num}", [values], value_input_option="USER_ENTERED"))
