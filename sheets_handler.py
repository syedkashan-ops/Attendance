...
def get_worksheet():
    ...
    if not values:
        ...
        if not any(str(x).strip() for x in values[0]):
            ws.update(
                "A1",
                [HEADERS],
...
def _row_number_for_visit(ws, visit_id):
    # Get the Visit ID column only; row 1 is header.
...
def complete_out_visit(
    ...
    in_lat = float(row["IN Latitude"])
    in_lon = float(row["IN Longitude"])
    out_lat = float(location["latitude"])
    out_lon = float(location["longitude"])
    ...
    in_time = datetime.strptime(
        str(row["IN Date/Time"]),
        "%Y-%m-%d %H:%M:%S",
    ).replace(tzinfo=ZoneInfo("Asia/Karachi"))
    ...
    mm = (seconds % 3600) // 60
    ss = seconds % 60
    time_spent = f"{hh:02d}:{mm:02d}:{ss:02d}"
    ...
    # One API call instead of nine separate calls.
    ws.update(
        f"K{row_num}:S{row_num}",
        [updates],
        value_input_option="USER_ENTERED",
    )
    ...
    result = row.copy()
    result.update({
        "OUT Date/Time": updates[0],
        "OUT Latitude": updates[1],
        "OUT Longitude": updates[2],
        "OUT GPS Accuracy": updates[3],
        "IN-OUT GPS Distance (m)": updates[4],
        "GPS Status": updates[5],
        "Time Spent": updates[6],
        "Time Spent Hours": updates[7],
        "Status": "Completed",
    })
    return result
